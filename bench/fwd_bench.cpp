...
HomogeneousTransitions * create_homogeneous_transitions(int n_states, double alpha, int k) {
  ...
  for (int i = 0; i < n_states; ++i) {
    ...
    state_i = new AutoCorr(n_states, i, n_targets, targets, alpha);
...
NonHomogeneousTransitions * create_nonhomogeneous_transitions(int n_states, int covar_slot, int k) {
  ...
  if (n_states == 1) {
    ...
    AutoCorrCovar * state = new AutoCorrCovar(n_states, 0, n_targets, &target, covar_slot);
    result->insert(state);
    return result;
  }
  ...
  for (int i = 0; i < n_states; ++i) {
    ...
    state_i = new AutoCorrCovar(n_states, i, n_targets, targets, covar_slot);
    result->insert(state_i);
  }
