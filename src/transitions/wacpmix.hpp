...
// Mixture of Auto-correlation and prior (from covar) w/ weights
//
// Parameters:
...
//
// Two covar slots:
// - prior covar slot must have dim == 2 [self, other]
//   priors are in log-space
// - weight covar slot must have dim == n_targets
//   weights are in log-space
//   exp(weights) must sum to one
//   weight for 'self' must be -Inf (exp(weight) = 0)
class WACPMix : public TransitionFunction {
public:

  WACPMix(int n_states, int stateID, int n_targets, int * targets, double alpha = 0.5, double gamma = 0.5, int prior_covar_slot = 0, int weight_covar_slot = 1, int max_iters = 100, double tolerance = 1e-4) : TransitionFunction(n_states, stateID, n_targets, targets), _prior_covar_slot(prior_covar_slot), _weight_covar_slot(weight_covar_slot), _alpha(alpha), _gamma(gamma), _is_fixed_alpha(false), _max_iters(100), _tolerance(tolerance) {
  ...
  virtual double log_probability(Iter const & iter, int target) const {
    if (_valid_states[target]) {
      if (target == _stateID) {
        double log_prior = iter.covar_i(_prior_covar_slot, 0);
        
        return QHMM_logsum(_log_acor_self, log_prior + _log_prior_weight);
      } else {
        int target_idx = _rev_target_map[target];
        double log_prior = iter.covar_i(_prior_covar_slot, 1);
        double log_weight = iter.covar_i(_prior_covar_slot, target_idx);
        
        return QHMM_logsum(_log_acor_other, log_prior + _log_prior_weight) + log_weight;
      }
    } else
  ...
  virtual void updateParams(EMSequences * sequences, std::vector<TransitionFunction*> * group) {
    ...
    for (tf_it = group->begin(); tf_it != group->end(); ++tf_it) {
      WACPMix * tf = (WACPMix*) (*tf_it)->inner();
  ...
  // TODO: enforce covar slot dimensions
  virtual bool setCovarSlots(int * slots, int length) {
    if (length != 2)
      return false;
    if (slots[0] < 0 || slots[1] < 0)
      return false;
    _prior_covar_slot = slots[0];
    _weight_covar_slot = slots[1];
    return true;
  }

private:
  int _prior_covar_slot;
  int _weight_covar_slot;
  double _alpha;
  double _gamma;
  ...
  void compute_fx_gx(double alpha, TransitionPosteriorIterator * piter, std::vector<TransitionFunction*> * group, double * out_fx, double * out_gx) {
    ...
    do {
      ...
      double log_prior_self = piter->covar_i(_prior_covar_slot, 0);
      double log_prior_other = piter->covar_i(_prior_covar_slot, 1);
      double prior_self = exp(log_prior_self);
      double prior_other = exp(log_prior_other);
      ...
      for (int tgt_idx = 0; tgt_idx < _n_targets; ++tgt_idx) {
        ...
        for (unsigned int gidx = 0; gidx < group->size(); ++gidx) {
           ...
           */
          WACPMix * gState = (WACPMix*) (*group)[gidx]->inner();
          
          bool is_self = gState->_stateID == gState->_targets[tgt_idx];
          if (is_self) {
            double denom = (_gamma * alpha + (1.0 - _gamma) * prior_self);
            fx += post / denom;
            gx -= post * _gamma / (denom * denom);
          } else {
            double denom = (_gamma * (1.0 - alpha) + (1.0 - _gamma) * prior_other);
            fx -= post / denom;
            gx -= post * _gamma / (denom * denom);
          }
          
          if (QHMM_isinf(gx) || QHMM_isinf(gx)) {
            double prior = (is_self ? prior_self : prior_other);
