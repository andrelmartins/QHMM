...
public:
  ...
  virtual double log_probability(Iter const & iter) const {
    ...
    if (x_low <= 0)
      return pgamma(x_high, _shape, _scale, TRUE, TRUE);
    
    /* Gamma tricks from Charle's code */
    int useLowerTail = (round(pgamma(x_low, _shape, _scale, FALSE, TRUE))==0); // Check for underflow.
    
    return expDif(pgamma(x_high, _shape, _scale, useLowerTail, TRUE),
                  pgamma(x_low, _shape, _scale, useLowerTail, TRUE));
  }
  ...
  virtual void updateParams(EMSequences * sequences, std::vector<EmissionFunction*> * group) {
    ...
    do {
      ...
      if (!R_FINITE(shape) || ISNAN(shape) || shape <= 0) {
    ...
    // check for weirdness
    if (shape > 1000 || ISNAN(shape) || !R_FINITE(shape))
...
private:
  ...
  double expDif(double pLs, double pGr) const {
    if(pGr > pLs)
